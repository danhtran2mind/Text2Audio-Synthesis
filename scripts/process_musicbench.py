...
def load_and_clean_dataset(dataset_id):
...
def move_and_cleanup_files(raw_data_dir, music_bench_dir, train_df, val_df, num_processes, dataset_dir):
    """Move files from datashare to music_bench, organize into train/test folders, and clean up."""
    # Create train and test directories
    os.makedirs(f"data/{dataset_dir}/audioset/train", exist_ok=True)
    os.makedirs(f"data/{dataset_dir}/audioset/test", exist_ok=True)
    ...
    train_tasks = [
        (os.path.join(datashare_dir, row["location"]),
         os.path.join(f"data/{dataset_dir}/audioset/train", os.path.basename(row["location"])),
         index)
        for index, row in train_df.iterrows()
    ]
    val_tasks = [
        (os.path.join(datashare_dir, row["location"]),
         os.path.join(f"data/{dataset_dir}/audioset/test", os.path.basename(row["location"])),
         index)
    ...
    train_dir = f"data/{dataset_dir}/audioset/train"
    ...
    test_dir = f"data/{dataset_dir}/audioset/test"
...
def write_json_files(train_data, val_data, dataset_dir):
    """Write train and validation data to JSON files."""
    os.makedirs(f"data/{dataset_dir}/audioset", exist_ok=True)
    with open(f"data/{dataset_dir}/audioset/train.json", "w") as f:
        json.dump({"data": train_data}, f, indent=4)
    with open(f"data/{dataset_dir}/audioset/test.json", "w") as f:
        json.dump({"data": val_data}, f, indent=4)


def create_dataset_root_json(dataset_dir):
    ...
    os.makedirs(f"data/{dataset_dir}/metadata", exist_ok=True)
    with open(f"data/{dataset_dir}/metadata/dataset_root.json", "w") as f:
        json.dump(dataset_root, f, indent=4)


def main(arg_process, dataset_id):
    ...
    # Process dataset_id to create directory name (replace '/' with '-')
    dataset_dir = dataset_id.replace('/', '-')
    raw_data_dir = f"data/datasets--{dataset_dir}"
    music_bench_dir = f"./data/{dataset_dir}/audioset/music_bench"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_id", type=str, default="amaai-lab/MusicBench",
